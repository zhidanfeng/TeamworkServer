...
//    }
    ...
    @GetMapping(value = "/listbypage")
    public Result getByPage(HttpServletRequest request) {
        int offsetPage = Integer.valueOf(request.getParameter("offsetPage"));
        int pageSize = Integer.valueOf(request.getParameter("pageSize"));
        PageRequestVO vo = new PageRequestVO();
        vo.setOffsetPage(offsetPage);
        vo.setPageSize(pageSize);
        if(request.getParameter("minRowId") != null) {
            Long minRowId = Long.valueOf(request.getParameter("minRowId"));
            vo.setMinRowId(minRowId);
        }
        if(request.getParameter("maxRowId") != null) {
            Long maxRowId = Long.valueOf(request.getParameter("maxRowId"));
            vo.setMaxRowId(maxRowId);
        }
        return resourceService.selectByPage(vo);
    }
